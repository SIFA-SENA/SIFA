...
public int registrarAprendiz(AprendizVo apr, FichaVo fic) throws SQLException{
    ...
    try{
        ...
        int rowsAffected = ps.executeUpdate(); //Ejecutar sentencia
        if (rowsAffected > 0) {
            // Éxito en la inserción
            System.out.println("Se registró el aprendiz correctamente");
            resultado = 1; // 1 indica éxito
        } else {
            // Fallo en la inserción
            System.out.println("No se pudo registrar el aprendiz");
            resultado = 0; // 0 indica falla
        }
...
public List<AprendizVo> filtroAprendiz(String texto) throws SQLException{
    List<AprendizVo> aprendiz=new ArrayList<>();
    sql="SELECT * from  aprendiz where idAprendiz like '%"+texto+"%' or nombreAprendiz like '%"+texto+"%' or tipodocAprendiz like '%"+texto+"%' or documentoAprendiz like '%"+texto+"%' or celularAprendiz like '%"+texto+"%' or correoAprendiz like '%"+texto+"%' or fechaNacimientoAprendiz like '%"+texto+"%' or estadoAprendiz like '%"+texto+"%' or observaciones like '%"+texto+"%' or idFichaFK like '%"+texto+"%'";
    try {
        con=Conexion.conectar();
        ps=con.prepareStatement(sql);
        rs=ps.executeQuery(sql);
