...
public class Aprendiz extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (accion!=null) {
         ...
         switch (accion) {
            ...
            break;
                  case "FormRegistrarAI":
            FormRegistrarAI(req,resp);
            break;
            ...
            break;
            case "listarAI":
            listarAprendizNA(req,resp);
            break;
            case "Buscar":
            filtroAprendiz(req, resp);
            break;
    ...
    private void FormRegistrarA(HttpServletRequest req, HttpServletResponse resp) {
      ...
      private void AbrirExcel(HttpServletRequest req, HttpServletResponse resp) {
        System.out.println("Entro al metodo abrir formulario");
        try{
            req.getRequestDispatcher("views/cargarExcel.jsp").forward(req, resp);
            System.out.println("El formulario ha sido abierto");
        }catch(Exception e){
            System.out.println("El formulario NO ha sido abierto"+e.getMessage().toString());
        }
    ...
    private void FormRegistrarAI(HttpServletRequest req, HttpServletResponse resp) {
        System.out.println("Entro al metodo abrir formulario");
        try{
            req.getRequestDispatcher("views/registrarAprendiz1.jsp").forward(req, resp);
            System.out.println("El formulario ha sido abierto");
        }catch(Exception e){
            System.out.println("El formulario NO ha sido abierto"+e.getMessage().toString());
        }
...
//---------------- ABRIR LISTAR APRENDICES NO ACTIVOS --------------------
int aprI;
 private void listarAprendizNA(HttpServletRequest req, HttpServletResponse resp) {
    
      aprI=Integer.parseInt(req.getParameter("idAI"));
       
          try {
             List aprendiz=ad.listarAprendizIna(aprI);
...
//-----------FILTRO BUSQUEDA APRENDIZ ------------------------------
 private void filtroAprendiz(HttpServletRequest req, HttpServletResponse resp) {
        try {
            String dato=req.getParameter("txtBuscar");
            List aprendiz=ad.filtroAprendiz(dato);
            req.setAttribute("aprendiz",aprendiz);
            req.getRequestDispatcher("views/listarxficha.jsp").forward(req, resp);
            System.out.println("Datos listados correctamente");
        } catch (Exception e) {
    ...
    /* private void procesarArchivoExcel(HttpServletRequest request, HttpServletResponse resp) throws IOException {
        try {
            // Obtén el archivo del HttpServletRequest
            Part filePart = request.getPart("excelFile");
            // Guarda los datos desde el archivo
            ad.guardarDatosDesdeExcel(filePart.getInputStream());
    
            resp.getWriter().write("Procesamiento de Excel exitoso");
        } catch (IOException e) {
            e.printStackTrace();
            resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error al procesar el archivo Excel: " + e.getMessage());
        } catch (SQLException e) {
            e.printStackTrace();
            resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error de SQL al procesar el archivo Excel: " + e.getMessage());
        }
    } */
}